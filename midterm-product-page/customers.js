  ...
  $.ajax({
    ...
    for (let d in data) {
      ...
      let dataStr = `<tr id="row${d}">
                <td style="width:40px"><img src='image/icondelete.png' class='icon' onclick="deleteCustomer(${d})"></td>
                <td>${data[d].name}</td>
                <td>${data[d].email}</td>
                <td>${data[d].phone}</td>
            </tr>`;
...
function addCustomer() {
  ...
  let dataStr = `<tr id="row${newCustomerIndex}">
        <td style="width:40px"><img src='image/icondelete.png' class='icon' onclick="deleteCustomer(${newCustomerIndex})"></td>
